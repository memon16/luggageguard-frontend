...
function CheckoutForm({ booking, bookingId }: { booking: any; bookingId: string }) {
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'Error creando pago');
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between"><span className="text-gray-600">Reserva #</span><span className="font-semibold">{booking.id.slice(0, 8)}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">📍 Recogida</span><span>{booking.pickupAddress}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">🚚 Entrega</span><span>{booking.deliveryAddress}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">🎒 Maletas</span><span>{booking.numberOfBags}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">📅 Días</span><span>{booking.storageDays}</span></div>
